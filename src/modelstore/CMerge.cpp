 ...
 */
static std::map<objectid_t, CMerge*> t_store;
/*
 *
...
static int save(tSimObj* obj, uint64_t aCycle, ... ) {
    int result = 0;
// User-Defined-Code: save-merge
// End-Of-UDC: save-merge
    return (result);
}
...
//
// **************************************************************************
bool CMerge::DefaultSigHandler(tSig *sig) {
    bool retval = false;
...
//
// **************************************************************************
tMsg* CMerge::DefaultMsgHandler(tMsg *msg) {
    tMsg* retval = 0;
...
// **************************************************************************
//
//  Method-Name   : create_merge_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_merge_obj(objectid_t  oid) {
    CMerge* newmerge = new CMerge;

    if (newmerge != 0) {
        tSimObj* newobj = newmerge;

        newobj->type        = IDO_MERGE;
        newobj->objid       = oid;
        newobj->parent      = 0;
        ...
        if (0xc0000000 & oid) {
            t_store.insert(std::pair<templateid_t, CMerge*>(oid, newmerge));
        }
    } else {
    }
    return ((tSimObj*)newmerge);
}
// **************************************************************************
//
//  Method-Name   : create_new_merge_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_merge_obj(objectid_t  oid) {
    CMerge* newmerge = new CMerge;
...
// **************************************************************************
//
//  Method-Name   : create_new_merge_obj_from_template()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_merge_obj_from_template(templateid_t  tid, objectid_t  oid) {
    CMerge* newmerge = 0;
    std::map<objectid_t, CMerge*>::iterator found;
...
tObjLib merge_factory= {0, 0, 0, IDO_MERGE, create_merge_obj, create_new_merge_obj, create_new_merge_obj_from_template};
