 ...
 */
static std::map<objectid_t, CIntersection*> t_store;
/*
 *
...
static int save(tSimObj* obj, uint64_t aCycle, ... ) {
    int result = 0;
// User-Defined-Code: save-intersection
// End-Of-UDC: save-intersection
    return (result);
}
...
static int setvalue(tSimObj * obj, valueid_t  valueid, valueindex_t  valueindex, const tVariant& value) {
    int err = 0;
    CIntersection* intersection_var = (CIntersection*)obj;
...
static void init_object(tSimObj * obj, uint64_t  aCycle) {
    CIntersection* thisobj = (CIntersection*)obj;
    /*
...
//
// **************************************************************************
bool CIntersection::DefaultSigHandler(tSig *sig) {
    bool retval = false;
...
//
// **************************************************************************
tMsg* CIntersection::DefaultMsgHandler(tMsg *msg) {
    tMsg* retval = 0;
...
// **************************************************************************
//
//  Method-Name   : create_intersection_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_intersection_obj(objectid_t  oid) {
    CIntersection* newintersection = new CIntersection;

    if (newintersection != 0) {
        tSimObj* newobj = newintersection;

        newobj->type        = IDO_INTERSECTION;
        newobj->objid       = oid;
        newobj->parent      = 0;
        ...
        if (0xc0000000 & oid) {
            t_store.insert(std::pair<templateid_t, CIntersection*>(oid, newintersection));
        }
    } else {
    }
    return ((tSimObj*)newintersection);
}
// **************************************************************************
//
//  Method-Name   : create_new_intersection_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_intersection_obj(objectid_t  oid) {
    CIntersection* newintersection = new CIntersection;
    ...
    if (newintersection != 0) {
        ...
        stdb_createobj(oid, IDO_INTERSECTION);
        //
...
// **************************************************************************
//
//  Method-Name   : create_new_intersection_obj_from_template()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_intersection_obj_from_template(templateid_t  tid, objectid_t  oid) {
    CIntersection* newintersection = 0;
    std::map<objectid_t, CIntersection*>::iterator found;

    found = t_store.find(tid);
    if (found != t_store.end()) {
        newintersection = new CIntersection;
        if (newintersection != 0) {
            tSimObj* newobj = newintersection;
...
tObjLib intersection_factory= {0, 0, 0, IDO_INTERSECTION, create_intersection_obj, create_new_intersection_obj, create_new_intersection_obj_from_template};
