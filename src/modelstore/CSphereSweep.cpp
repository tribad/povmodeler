 ...
 */
static std::map<objectid_t, CSphereSweep*> t_store;
/*
 *
...
static int save(tSimObj* obj, uint64_t aCycle, ... ) {
    int result = 0;
// User-Defined-Code: save-spheresweap
// End-Of-UDC: save-spheresweap
    return (result);
}
...
static int setvalue(tSimObj * obj, valueid_t  valueid, valueindex_t  valueindex, const tVariant& value) {
    int err = 0;
    CSphereSweep* spheresweap_var = (CSphereSweep*)obj;
...
static void init_object(tSimObj * obj, uint64_t  aCycle) {
    CSphereSweep* thisobj = (CSphereSweep*)obj;
    /*
...
//
// **************************************************************************
bool CSphereSweep::DefaultSigHandler(tSig *sig) {
    bool retval = false;
...
//
// **************************************************************************
tMsg* CSphereSweep::DefaultMsgHandler(tMsg *msg) {
    tMsg* retval = 0;
...
// **************************************************************************
//
//  Method-Name   : create_spheresweap_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_spheresweap_obj(objectid_t  oid) {
    CSphereSweep* newspheresweap = new CSphereSweep;

    if (newspheresweap != 0) {
        tSimObj* newobj = newspheresweap;

        newobj->type        = IDO_SPHERESWEAP;
        newobj->objid       = oid;
        newobj->parent      = 0;
        ...
        if (0xc0000000 & oid) {
            t_store.insert(std::pair<templateid_t, CSphereSweep*>(oid, newspheresweap));
        }
    } else {
    }
    return ((tSimObj*)newspheresweap);
}
// **************************************************************************
//
//  Method-Name   : create_new_spheresweap_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_spheresweap_obj(objectid_t  oid) {
    CSphereSweep* newspheresweap = new CSphereSweep;
    ...
    if (newspheresweap != 0) {
        ...
        stdb_createobj(oid, IDO_SPHERESWEAP);
        //
...
// **************************************************************************
//
//  Method-Name   : create_new_spheresweap_obj_from_template()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_spheresweap_obj_from_template(templateid_t  tid, objectid_t  oid) {
    CSphereSweep* newspheresweap = 0;
    std::map<objectid_t, CSphereSweep*>::iterator found;

    found = t_store.find(tid);
    if (found != t_store.end()) {
        newspheresweap = new CSphereSweep;
        if (newspheresweap != 0) {
            tSimObj* newobj = newspheresweap;
...
tObjLib spheresweap_factory= {0, 0, 0, IDO_SPHERESWEAP, create_spheresweap_obj, create_new_spheresweap_obj, create_new_spheresweap_obj_from_template};
