 ...
 */
static std::map<objectid_t, CHalo*> t_store;
/*
 *
...
static int save(tSimObj* obj, uint64_t aCycle, ... ) {
    int result = 0;
// User-Defined-Code: save-halo
// End-Of-UDC: save-halo
    return (result);
}
...
//
// **************************************************************************
bool CHalo::DefaultSigHandler(tSig *sig) {
    bool retval = false;
...
//
// **************************************************************************
tMsg* CHalo::DefaultMsgHandler(tMsg *msg) {
    tMsg* retval = 0;
...
// **************************************************************************
//
//  Method-Name   : create_halo_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_halo_obj(objectid_t  oid) {
    CHalo* newhalo = new CHalo;

    if (newhalo != 0) {
        tSimObj* newobj = newhalo;

        newobj->type        = IDO_HALO;
        newobj->objid       = oid;
        newobj->parent      = 0;
        ...
        if (0xc0000000 & oid) {
            t_store.insert(std::pair<templateid_t, CHalo*>(oid, newhalo));
        }
    } else {
    }
    return ((tSimObj*)newhalo);
}
// **************************************************************************
//
//  Method-Name   : create_new_halo_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_halo_obj(objectid_t  oid) {
    CHalo* newhalo = new CHalo;
...
// **************************************************************************
//
//  Method-Name   : create_new_halo_obj_from_template()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_halo_obj_from_template(templateid_t  tid, objectid_t  oid) {
    CHalo* newhalo = 0;
    std::map<objectid_t, CHalo*>::iterator found;
...
tObjLib halo_factory= {0, 0, 0, IDO_HALO, create_halo_obj, create_new_halo_obj, create_new_halo_obj_from_template};
