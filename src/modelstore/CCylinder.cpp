 ...
 */
static std::map<objectid_t, CCylinder*> t_store;
/*
 *
...
static int save(tSimObj* obj, uint64_t aCycle, ... ) {
    int result = 0;
// User-Defined-Code: save-cylinder
// End-Of-UDC: save-cylinder
    return (result);
}
...
static int setvalue(tSimObj * obj, valueid_t  valueid, valueindex_t  valueindex, const tVariant& value) {
    int err = 0;
    CCylinder* cylinder_var = (CCylinder*)obj;
...
static void init_object(tSimObj * obj, uint64_t  aCycle) {
    CCylinder* thisobj = (CCylinder*)obj;
    /*
...
//
// **************************************************************************
bool CCylinder::DefaultSigHandler(tSig *sig) {
    bool retval = false;
...
//
// **************************************************************************
tMsg* CCylinder::DefaultMsgHandler(tMsg *msg) {
    tMsg* retval = 0;
...
// **************************************************************************
//
//  Method-Name   : create_cylinder_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_cylinder_obj(objectid_t  oid) {
    CCylinder* newcylinder = new CCylinder;

    if (newcylinder != 0) {
        tSimObj* newobj = newcylinder;

        newobj->type        = IDO_CYLINDER;
        newobj->objid       = oid;
        newobj->parent      = 0;
        ...
        if (0xc0000000 & oid) {
            t_store.insert(std::pair<templateid_t, CCylinder*>(oid, newcylinder));
        }
    } else {
    }
    return ((tSimObj*)newcylinder);
}
// **************************************************************************
//
//  Method-Name   : create_new_cylinder_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_cylinder_obj(objectid_t  oid) {
    CCylinder* newcylinder = new CCylinder;
    ...
    if (newcylinder != 0) {
        ...
        stdb_createobj(oid, IDO_CYLINDER);
        //
...
// **************************************************************************
//
//  Method-Name   : create_new_cylinder_obj_from_template()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_cylinder_obj_from_template(templateid_t  tid, objectid_t  oid) {
    CCylinder* newcylinder = 0;
    std::map<objectid_t, CCylinder*>::iterator found;

    found = t_store.find(tid);
    if (found != t_store.end()) {
        newcylinder = new CCylinder;
        if (newcylinder != 0) {
            tSimObj* newobj = newcylinder;
...
tObjLib cylinder_factory= {0, 0, 0, IDO_CYLINDER, create_cylinder_obj, create_new_cylinder_obj, create_new_cylinder_obj_from_template};
