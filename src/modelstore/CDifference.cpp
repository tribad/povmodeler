 ...
 */
static std::map<objectid_t, CDifference*> t_store;
/*
 *
...
static int save(tSimObj* obj, uint64_t aCycle, ... ) {
    int result = 0;
// User-Defined-Code: save-difference
// End-Of-UDC: save-difference
    return (result);
}
...
static int setvalue(tSimObj * obj, valueid_t  valueid, valueindex_t  valueindex, const tVariant& value) {
    int err = 0;
    CDifference* difference_var = (CDifference*)obj;
...
static void init_object(tSimObj * obj, uint64_t  aCycle) {
    CDifference* thisobj = (CDifference*)obj;
    /*
...
//
// **************************************************************************
bool CDifference::DefaultSigHandler(tSig *sig) {
    bool retval = false;
...
//
// **************************************************************************
tMsg* CDifference::DefaultMsgHandler(tMsg *msg) {
    tMsg* retval = 0;
...
// **************************************************************************
//
//  Method-Name   : create_difference_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_difference_obj(objectid_t  oid) {
    CDifference* newdifference = new CDifference;

    if (newdifference != 0) {
        tSimObj* newobj = newdifference;

        newobj->type        = IDO_DIFFERENCE;
        newobj->objid       = oid;
        newobj->parent      = 0;
        ...
        if (0xc0000000 & oid) {
            t_store.insert(std::pair<templateid_t, CDifference*>(oid, newdifference));
        }
    } else {
    }
    return ((tSimObj*)newdifference);
}
// **************************************************************************
//
//  Method-Name   : create_new_difference_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_difference_obj(objectid_t  oid) {
    CDifference* newdifference = new CDifference;
    ...
    if (newdifference != 0) {
        ...
        stdb_createobj(oid, IDO_DIFFERENCE);
        //
...
// **************************************************************************
//
//  Method-Name   : create_new_difference_obj_from_template()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_difference_obj_from_template(templateid_t  tid, objectid_t  oid) {
    CDifference* newdifference = 0;
    std::map<objectid_t, CDifference*>::iterator found;

    found = t_store.find(tid);
    if (found != t_store.end()) {
        newdifference = new CDifference;
        if (newdifference != 0) {
            tSimObj* newobj = newdifference;
...
tObjLib difference_factory= {0, 0, 0, IDO_DIFFERENCE, create_difference_obj, create_new_difference_obj, create_new_difference_obj_from_template};
