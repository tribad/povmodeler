 ...
 */
static std::map<objectid_t, CPolygon*> t_store;
/*
 *
...
static int save(tSimObj* obj, uint64_t aCycle, ... ) {
    int result = 0;
// User-Defined-Code: save-polygon
// End-Of-UDC: save-polygon
    return (result);
}
...
static int setvalue(tSimObj * obj, valueid_t  valueid, valueindex_t  valueindex, const tVariant& value) {
    int err = 0;
    CPolygon* polygon_var = (CPolygon*)obj;
...
//
// **************************************************************************
bool CPolygon::DefaultSigHandler(tSig *sig) {
    bool retval = false;
...
//
// **************************************************************************
tMsg* CPolygon::DefaultMsgHandler(tMsg *msg) {
    tMsg* retval = 0;
...
// **************************************************************************
//
//  Method-Name   : create_polygon_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_polygon_obj(objectid_t  oid) {
    CPolygon* newpolygon = new CPolygon;

    if (newpolygon != 0) {
        tSimObj* newobj = newpolygon;

        newobj->type        = IDO_POLYGON;
        newobj->objid       = oid;
        newobj->parent      = 0;
        ...
        if (0xc0000000 & oid) {
            t_store.insert(std::pair<templateid_t, CPolygon*>(oid, newpolygon));
        }
    } else {
    }
    return ((tSimObj*)newpolygon);
}
// **************************************************************************
//
//  Method-Name   : create_new_polygon_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_polygon_obj(objectid_t  oid) {
    CPolygon* newpolygon = new CPolygon;
    ...
    if (newpolygon != 0) {
        ...
        stdb_createobj(oid, IDO_POLYGON);
        //
...
// **************************************************************************
//
//  Method-Name   : create_new_polygon_obj_from_template()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_polygon_obj_from_template(templateid_t  tid, objectid_t  oid) {
    CPolygon* newpolygon = 0;
    std::map<objectid_t, CPolygon*>::iterator found;
...
tObjLib polygon_factory= {0, 0, 0, IDO_POLYGON, create_polygon_obj, create_new_polygon_obj, create_new_polygon_obj_from_template};
