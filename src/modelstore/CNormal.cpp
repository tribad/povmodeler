 ...
 */
static std::map<objectid_t, CNormal*> t_store;
/*
 *
...
static int save(tSimObj* obj, uint64_t aCycle, ... ) {
    int result = 0;
// User-Defined-Code: save-normal
// End-Of-UDC: save-normal
    return (result);
}
...
static int setvalue(tSimObj * obj, valueid_t  valueid, valueindex_t  valueindex, const tVariant& value) {
    int err = 0;
    CNormal* normal_var = (CNormal*)obj;
...
//
// **************************************************************************
bool CNormal::DefaultSigHandler(tSig *sig) {
    bool retval = false;
...
//
// **************************************************************************
tMsg* CNormal::DefaultMsgHandler(tMsg *msg) {
    tMsg* retval = 0;
...
// **************************************************************************
//
//  Method-Name   : create_normal_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_normal_obj(objectid_t  oid) {
    CNormal* newnormal = new CNormal;

    if (newnormal != 0) {
        tSimObj* newobj = newnormal;

        newobj->type        = IDO_NORMAL;
        newobj->objid       = oid;
        newobj->parent      = 0;
        ...
        if (0xc0000000 & oid) {
            t_store.insert(std::pair<templateid_t, CNormal*>(oid, newnormal));
        }
    } else {
    }
    return ((tSimObj*)newnormal);
}
// **************************************************************************
//
//  Method-Name   : create_new_normal_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_normal_obj(objectid_t  oid) {
    CNormal* newnormal = new CNormal;
...
// **************************************************************************
//
//  Method-Name   : create_new_normal_obj_from_template()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_normal_obj_from_template(templateid_t  tid, objectid_t  oid) {
    CNormal* newnormal = 0;
    std::map<objectid_t, CNormal*>::iterator found;

    found = t_store.find(tid);
    if (found != t_store.end()) {
        newnormal = new CNormal;
        if (newnormal != 0) {
            tSimObj* newobj = newnormal;
...
tObjLib normal_factory= {0, 0, 0, IDO_NORMAL, create_normal_obj, create_new_normal_obj, create_new_normal_obj_from_template};
