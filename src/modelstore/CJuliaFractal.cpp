 ...
 */
static std::map<objectid_t, CJuliaFractal*> t_store;
/*
 *
...
static int save(tSimObj* obj, uint64_t aCycle, ... ) {
    int result = 0;
// User-Defined-Code: save-juliafractal
// End-Of-UDC: save-juliafractal
    return (result);
}
...
static int setvalue(tSimObj * obj, valueid_t  valueid, valueindex_t  valueindex, const tVariant& value) {
    int err = 0;
    CJuliaFractal* juliafractal_var = (CJuliaFractal*)obj;
...
static void init_object(tSimObj * obj, uint64_t  aCycle) {
    CJuliaFractal* thisobj = (CJuliaFractal*)obj;
    /*
...
//
// **************************************************************************
bool CJuliaFractal::DefaultSigHandler(tSig *sig) {
    bool retval = false;
...
//
// **************************************************************************
tMsg* CJuliaFractal::DefaultMsgHandler(tMsg *msg) {
    tMsg* retval = 0;
...
// **************************************************************************
//
//  Method-Name   : create_juliafractal_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_juliafractal_obj(objectid_t  oid) {
    CJuliaFractal* newjuliafractal = new CJuliaFractal;

    if (newjuliafractal != 0) {
        tSimObj* newobj = newjuliafractal;

        newobj->type        = IDO_JULIAFRACTAL;
        newobj->objid       = oid;
        newobj->parent      = 0;
        ...
        if (0xc0000000 & oid) {
            t_store.insert(std::pair<templateid_t, CJuliaFractal*>(oid, newjuliafractal));
        }
    } else {
    }
    return ((tSimObj*)newjuliafractal);
}
// **************************************************************************
//
//  Method-Name   : create_new_juliafractal_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_juliafractal_obj(objectid_t  oid) {
    CJuliaFractal* newjuliafractal = new CJuliaFractal;
    ...
    if (newjuliafractal != 0) {
        ...
        stdb_createobj(oid, IDO_JULIAFRACTAL);
        //
...
// **************************************************************************
//
//  Method-Name   : create_new_juliafractal_obj_from_template()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_juliafractal_obj_from_template(templateid_t  tid, objectid_t  oid) {
    CJuliaFractal* newjuliafractal = 0;
    std::map<objectid_t, CJuliaFractal*>::iterator found;

    found = t_store.find(tid);
    if (found != t_store.end()) {
        newjuliafractal = new CJuliaFractal;
        if (newjuliafractal != 0) {
            tSimObj* newobj = newjuliafractal;
...
tObjLib juliafractal_factory= {0, 0, 0, IDO_JULIAFRACTAL, create_juliafractal_obj, create_new_juliafractal_obj, create_new_juliafractal_obj_from_template};
