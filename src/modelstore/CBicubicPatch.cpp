 ...
 */
static std::map<objectid_t, CBicubicPatch*> t_store;
/*
 *
...
static int save(tSimObj* obj, uint64_t aCycle, ... ) {
    int result = 0;
// User-Defined-Code: save-bicubicpatch
// End-Of-UDC: save-bicubicpatch
    return (result);
}
...
static int setvalue(tSimObj * obj, valueid_t  valueid, valueindex_t  valueindex, const tVariant& value) {
    int err = 0;
    CBicubicPatch* bicubicpatch_var = (CBicubicPatch*)obj;
...
static void init_object(tSimObj * obj, uint64_t  aCycle) {
    CBicubicPatch* thisobj = (CBicubicPatch*)obj;
    /*
...
//
// **************************************************************************
bool CBicubicPatch::DefaultSigHandler(tSig *sig) {
    bool retval = false;
...
//
// **************************************************************************
tMsg* CBicubicPatch::DefaultMsgHandler(tMsg *msg) {
    tMsg* retval = 0;
...
// **************************************************************************
//
//  Method-Name   : create_bicubicpatch_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_bicubicpatch_obj(objectid_t  oid) {
    CBicubicPatch* newbicubicpatch = new CBicubicPatch;

    if (newbicubicpatch != 0) {
        tSimObj* newobj = newbicubicpatch;

        newobj->type        = IDO_BICUBICPATCH;
        newobj->objid       = oid;
        newobj->parent      = 0;
        ...
        if (0xc0000000 & oid) {
            t_store.insert(std::pair<templateid_t, CBicubicPatch*>(oid, newbicubicpatch));
        }
    } else {
    }
    return ((tSimObj*)newbicubicpatch);
}
// **************************************************************************
//
//  Method-Name   : create_new_bicubicpatch_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_bicubicpatch_obj(objectid_t  oid) {
    CBicubicPatch* newbicubicpatch = new CBicubicPatch;
    ...
    if (newbicubicpatch != 0) {
        ...
        stdb_createobj(oid, IDO_BICUBICPATCH);
        //
...
// **************************************************************************
//
//  Method-Name   : create_new_bicubicpatch_obj_from_template()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_bicubicpatch_obj_from_template(templateid_t  tid, objectid_t  oid) {
    CBicubicPatch* newbicubicpatch = 0;
    std::map<objectid_t, CBicubicPatch*>::iterator found;

    found = t_store.find(tid);
    if (found != t_store.end()) {
        newbicubicpatch = new CBicubicPatch;
        if (newbicubicpatch != 0) {
            tSimObj* newobj = newbicubicpatch;
...
tObjLib bicubicpatch_factory= {0, 0, 0, IDO_BICUBICPATCH, create_bicubicpatch_obj, create_new_bicubicpatch_obj, create_new_bicubicpatch_obj_from_template};
