 ...
 */
static std::map<objectid_t, CSphere*> t_store;
/*
 *
...
static int save(tSimObj* obj, uint64_t aCycle, ... ) {
    int result = 0;
// User-Defined-Code: save-sphere
// End-Of-UDC: save-sphere
    return (result);
}
...
static int setvalue(tSimObj * obj, valueid_t  valueid, valueindex_t  valueindex, const tVariant& value) {
    int err = 0;
    CSphere* sphere_var = (CSphere*)obj;
...
//
// **************************************************************************
bool CSphere::DefaultSigHandler(tSig *sig) {
    bool retval = false;
...
//
// **************************************************************************
tMsg* CSphere::DefaultMsgHandler(tMsg *msg) {
    tMsg* retval = 0;
...
// **************************************************************************
//
//  Method-Name   : create_sphere_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_sphere_obj(objectid_t  oid) {
    CSphere* newsphere = new CSphere;

    if (newsphere != 0) {
        tSimObj* newobj = newsphere;

        newobj->type        = IDO_SPHERE;
        newobj->objid       = oid;
        newobj->parent      = 0;
        ...
        if (0xc0000000 & oid) {
            t_store.insert(std::pair<templateid_t, CSphere*>(oid, newsphere));
        }
    } else {
    }
    return ((tSimObj*)newsphere);
}
// **************************************************************************
//
//  Method-Name   : create_new_sphere_obj()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_sphere_obj(objectid_t  oid) {
    CSphere* newsphere = new CSphere;
...
// **************************************************************************
//
//  Method-Name   : create_new_sphere_obj_from_template()
//
//  Generated source code.
//
// **************************************************************************
static tSimObj* create_new_sphere_obj_from_template(templateid_t  tid, objectid_t  oid) {
    CSphere* newsphere = 0;
    std::map<objectid_t, CSphere*>::iterator found;
...
tObjLib sphere_factory= {0, 0, 0, IDO_SPHERE, create_sphere_obj, create_new_sphere_obj, create_new_sphere_obj_from_template};
